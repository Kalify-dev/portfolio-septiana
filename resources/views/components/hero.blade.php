...
<section class="relative min-h-screen w-full flex items-center justify-center overflow-hidden bg-fixed bg-center bg-cover" 
         style="background-image: url('{{ $profile && $profile->hero_image ? asset('storage/' . $profile->hero_image) : ($profile && $profile->image ? asset('storage/' . $profile->image) : asset('assets/images/bg-login.jpg')) }}');">
