...
<section class="relative min-h-screen w-full flex items-center justify-center overflow-hidden bg-fixed bg-center bg-cover" 
         style="background-image: url('{{ $quote && $quote->foto_hero ? asset('storage/' . $quote->foto_hero) : ($profile && $profile->image ? asset('storage/' . $profile->image) : asset('img/FOTO YANG DIA SUKA.jpeg')) }}');">
